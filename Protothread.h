...
public:
    ...
    inline void Stop() { _ptLine = LineNumberInvalid; }

    // Return true if the protothread is running or waiting, false if it has
    // ended or exited.
    inline bool IsRunning() const { return _ptLine != LineNumberInvalid; }
...
protected:
    ...
    static constexpr LineNumber LineNumberInvalid = static_cast<LineNumber>(-1);
...
// Call a child protothread and wait until it completes
...
#define PT_YIELD()                                                          \
    do {                                                                    \
        ...
        case __LINE__:                                                      \
            if (!ptYielded) {                                               \
                return true; /* still running, yielded */                   \
            }                                                               \
    } while (0)
